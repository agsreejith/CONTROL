...
;
; PURPOSE:
;      Returns light curves in predefined wavelength regions
;      
; CALLING SEQUENCE:
;     CONTROL_LIGHT_CURVE,file_list,lightcurve,wave_region=wave_region
;
; INPUTS:
;      fial_list   = The list of files needed to produce the light curves.
;
; OPTIONAL INPUTS
;      wave_region = Wavelength region of intrest outside default.
;                    Options are MgII, MgI, Fe II, individually or grouped.
;  
; OUTPUT:
;      lightcurve = Light curve in the following wavelegth regions.
;                   1. Full wavelength band (white light)
;                   2. 
;
; PROCEDURE:
...
pro control_light_curve,file_list,lightcurve,wave_region=wave_region
  ...
  for i=0,n_elements(file_list)-1 do begin
    ...
    if  fix(SXPAR(spectrum_hdr, 'FCALFLG',MISSING=-1)) eq -1 then begin
      ...
      logprint,'Press q to skip the creation of default light curves.'$
                +' Press any key to continue with light curve generation.'
      R = GET_KBRD()
    ...
    if (tag_exist(wclf_spectrum_file,'flux')eq 1) then spectra_1dwf=wclf_spectrum_file.flux $
    else if (tag_exist(wclf_spectrum_file,'counts') eq 1) then spectra_1dwf=wclf_spectrum_file.counts
...
lightcurve={time:reform(lc_all[0,*]),full_data:reform(lc_all[1,*]),full_error:reform(lc_all[2,*])$
            ,short_data:reform(lc1[1,*]),short_error:reform(lc1[2,*]),middle_data:reform(lc2[1,*])$
            ,middle_error:reform(lc2[2,*]),long_data:reform(lc3[1,*]),long_error:reform(lc3[2,*])}
if mg2 ge 0 then begin
