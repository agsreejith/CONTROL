...
;
; INPUTS:
;      bias_list = The file containing list of bias file locations. 
; 
; OPTIONAL INPUTS
...
;print,'control bias'
err = ''
  if N_params() LT 2 then begin             ;Need at least 4 parameters
    logprint,'CONTROL_BIAS_COMBINE: Syntax - control_bias_combine,bias_list,mbias',logonly = logonly
  ...
  if n eq 0 then begin
    ...
    if n_frames le 1 then begin
      logprint,'CONTROL BIAS COMBINE: Only one valid BIAS file found. Do you wnat to assume it as the MASTER BIAS?.'
      logprint,'Press q to skip this assumption. Press any key to continue with MASTER BIAS creation with one valid BIAS file.'
      R = GET_KBRD()
    ...
    logprint,'CONTROL BIAS COMBINE: Combining '+STRTRIM(STRING(n_frames),2)+' BIAS file to create MASTER BIAS using '+type+' method .'
    case type of
      'median':begin
                  if (idl_ver ge 8.3) then mbias_val = median(bias_arr,dimension=3,/even) else begin
      ...
      'mean' :begin
                if (idl_ver ge 8.3) then mbias_val = mean(bias_arr,dimension=3,/NAN) else begin
                  totalImage = Total(bias_arr, 3,/NAN)
    ...
    logprint,'CONTROL BIAS COMBINE: Saturated pixels (values above '+STRTRIM(STRING(sat_value),2)+') and pixels that deviate by '+STRTRIM(STRING(threshold),2)+' sigma from mean of  MASTER BIAS have been flaged.'
