...
;      hbmap    =  map of cosmetic defects on the CCD as a 2-D array with the same sizes as the input CCD frame byte-type pixels (8-bit) and values of 1B for good pixels and 0B for bad onces.
;
; OPTIONAL OUTPUT:
;     type = 
;   
...
pro CONTROL_HOTBAD,input_file,hbmap,out,type
  ...
  if (elem eq 2) then begin 
    ...
    logprint,'CONTROL_HOTBAD input file diamension error',logonly = logonly
    message,'CONTROL_HOTBAD input file diamension error'
  ...
  for i = 0, nx-1 do begin
    bv = where(hbmap_nw[i,0:ny-1] eq 0)
    gv = where(hbmap_nw[i,0:ny-1] eq 1)
